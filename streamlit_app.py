...
# Fallback highlight colors (kept even if theme CSS is disabled)
HIGHLIGHT_BG = "#dfe8d2"
HIGHLIGHT_TEXT = "#2c2a26"
...
SESSION_MAP = {
    "Session 1": [
        "E-Juz 30, 29, 28, 27&26 - GIRLS",
        "D-Juz 30, 29,28&27 - GIRLS",
        "C-Juz 30,29&28 - GIRLS",
        "Surat Yusuf - GIRLS - ADULT",
        "Surat Al-Waqiah - GIRLS - ADULT",
        "Surat Al-Mulk - GIRLS - ADULT",
        "Surat Al-Kahf - GIRLS - ADULT",
        "Surat Al-Baqarah & Al Imran - GIRLS - ADULT",
        "Surat Al-Baqarah - GIRLS - ADULT",
    ],
    "Session 2": [
        "A-Juz 30 - GIRLS",
        "4 yrs: An-Nasr",
        "Grade 1: An-Naba",
        "Grade 3: Al-Mulk",
        "Grade 5: Al-Qalam - GIRLS",
        "Grade 7: Al-Hadid - GIRLS",
        "Grade 9: Al-Ahqaf - GIRLS",
        "Grade 12: Maryam - GIRLS",
    ],
    "Session 3": [
        "B-Juz 30&29 - GIRLS",
        "5 yrs: Az-Zalzala",
        "Grade 2: Al-Jumuah",
        "Grade 4: Al-Maarij",
        "Grade 6: Al-Hujurat - GIRLS",
        "Grade 8: Al-Fath - GIRLS",
        "Grade 10: Yaseen - GIRLS",
        "Grade 11: Az-zukhruf - GIRLS",
    ],
    "Session 4": [
        "E-Juz 30, 29, 28, 27&26 - BOYS",
        "Surat Yusuf - BOYS - ADULT",
        "Surat Al-Waqiah - BOYS - ADULT",
        "Surat Al-Mulk - BOYS - ADULT",
        "Surat Al-Kahf - BOYS - ADULT",
        "Surat Al-Baqarah & Al Imran - BOYS - ADULT",
        "Surat Al-Baqarah & Al Imran - BOYS",
        "Surat Al-Baqarah - BOYS",
        "Surat Al Imran - BOYS",
    ],
    "Session 5": [
        "C-Juz 30,29&28 - BOYS",
        "A-Juz 30 - BOYS",
        "B-Juz 30&29 - BOYS",
        "Grade 5: Al-Qalam - BOYS",
        "Grade 6: Al-Hujurat - BOYS",
        "Grade 7: Al-Hadid - BOYS",
        "Grade 8: Al-Fath - BOYS",
        "Grade 9: Al-Ahqaf - BOYS",
        "Grade 10: Yaseen - BOYS",
    ],
}
...
def session_label_for_title(title: str) -> str | None:
    normalized_title = _normalize_session_title(title)
    for session_name, titles in SESSION_MAP.items():
        lookup = {_normalize_session_title(t) for t in titles}
        if normalized_title in lookup:
            return session_name
    return None

def session_from_row(row: pd.Series) -> str:
    base = str(row.get("Grade", "")).strip()
    gender = str(row.get("Gender", "")).strip().upper()
    entry = str(row.get("Entry Type", "")).strip().upper()
    candidates = [base]
    if gender:
        candidates.append(f"{base} - {gender}")
        if entry == "ADULT":
            candidates.append(f"{base} - {gender} - ADULT")
    for title in candidates:
        session = session_label_for_title(title)
        if session:
            return session
    return ""

def append_session_label(title: str) -> str:
    session = session_label_for_title(title)
    if session:
    ...
    for grade in unique_grades:
        ...
        if not should_split(grade):
            ...
            if not child_sub.empty:
                ...
                tables[append_session_label(grade)] = child_sub[["Count", "First Name", "Last Name", "Email"]]
            if not adult_sub.empty:
                adult_sub.insert(0, "Count", range(1, len(adult_sub) + 1))
                title = f"{grade} - ADULT"
                tables[append_session_label(title)] = adult_sub[["Count", "First Name", "Last Name", "Email"]]
        else:
            # Gender Split Logic
            for gender in ["Boys", "Girls"]:
                ...
                if not child_sub.empty:
                    ...
                    tables[append_session_label(title)] = child_sub[["Count", "First Name", "Last Name", "Email"]]
                if not adult_sub.empty:
                    adult_sub.insert(0, "Count", range(1, len(adult_sub) + 1))
...
def _strip_session_suffix(title: str) -> str:
    return re.sub(r"\s*-\s*Session\s+\d+$", "", str(title)).strip()

def tables_to_session_single_sheet_bytes(tables: dict, session_name: str) -> bytes:
    output = BytesIO()
    session_titles = SESSION_MAP.get(session_name, [])
    report_lookup = {}
    for title, data in tables.items():
        if title == "Category_Counts":
            continue
        base_title = _strip_session_suffix(title)
        key = _normalize_session_title(base_title)
        if key not in report_lookup:
            report_lookup[key] = (base_title, data)

    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        workbook = writer.book
        title_fmt = workbook.add_format({"bold": True, "font_size": 16, "font_color": "#1F4E78", "align": "left"})
        sheet_name = session_name
        row_cursor = 0
        ws = None
        for title in session_titles:
            key = _normalize_session_title(title)
            if key not in report_lookup:
                continue
            _, data = report_lookup[key]
            export_data = data[["First Name", "Last Name"]].copy()
            export_data["Tested"] = ""
            export_data["Total Mark"] = ""
            export_data["Rank"] = ""
            export_data.to_excel(writer, sheet_name=sheet_name, startrow=row_cursor + 1, index=False)
            ws = writer.sheets[sheet_name]
            ws.write(row_cursor, 0, title, title_fmt)
            tested_col = export_data.columns.get_loc("Tested")
            data_start = row_cursor + 2
            for idx in range(len(export_data)):
                row = data_start + idx
                ws.insert_checkbox(row, tested_col, False)
            row_cursor += len(export_data) + 3
        if ws:
...
def labels_to_pdf_bytes(df: pd.DataFrame) -> bytes:
    ...
    for idx, row in df.iterrows():
        ...
        if col == 0:
            ...
            extra = extra_third_col if col == 2 else 0
            x += label_w + gap_x + extra
...
with tab_labels:
    ...
    label_view = results_df[["First Name", "Last Name", "Grade", "Gender", "Entry Type"]].copy()
    label_view["Session"] = label_view.apply(session_from_row, axis=1).str.replace("Session ", "")
    label_view["Label"] = (
    ...
    label_view["Label"] = label_view["Label"] + label_view["Session"].apply(lambda s: f" | Session {s}" if s else "")
...
st.sidebar.markdown("### Download Sessions")
for session_name in SESSION_MAP:
    safe_session = re.sub(r"[^A-Za-z0-9_-]+", "_", session_name).strip("_")
    st.sidebar.download_button(
        f"Download {session_name}",
        data=tables_to_session_single_sheet_bytes(reports, session_name),
        file_name=f"Quran_Contest_{safe_session}_{datetime.now().strftime('%Y%m%d')}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )
