...
#     <style>
#       .stApp {
#         background: radial-gradient(circle at 20% 10%, #f7f3ea 0%, #efe6d7 45%, #e6ebe6 100%);
#       }
...
#       .stSidebar,
#       .stSidebar > div {
#         background-color: #e7efe6 !important;
#         background-image: linear-gradient(180deg, #f4f8f2 0%, #e3efe7 100%) !important;
#         border-right: 1px solid #c9d6cc !important;
#       }
...
#         .stApp {
#           background: radial-gradient(circle at 20% 10%, #1b1f1b 0%, #22261f 45%, #1a1d1a 100%);
#           color: #e9e6df;
#         }
#         [data-testid="stSidebar"],
#         [data-testid="stSidebar"] > div,
...
#           background-image: linear-gradient(180deg, #222821 0%, #1b201c 100%) !important;
#           border-right: 1px solid #2f3a30 !important;
#         }
#       }
#     </style>
#     """,
...
#     if val in {"M", "MALE", "BOY", "BOYS"}:
#         return "Boys"
#     if val in {"F", "FEMALE", "GIRL", "GIRLS"}:
#         return "Girls"
#     return "Unknown"
...
#     if grade_match and int(grade_match.group(1)) <= 4:
#         return False
#     return True


# def extract_participants(df: pd.DataFrame, location_col: str | None = None) -> pd.DataFrame:
#     participants = []
#     slot_nums = sorted(
#         {
#             int(c.split("_")[0])
...
#             f_name = row.get(f"{s}_Name (First)")
#             if pd.isna(f_name):
#                 continue

#             l_name = row.get(f"{s}_Name (Last)", "")
#             email = row.get("Email", "N/A")

#             c_cat = row.get(f"{s}_Child Contest Options")
#             a_cat = row.get(f"{s}_Adult Contest Options")
...
#             if not has_value(cat):
#                 continue
#             entry_type = "Child" if c_has else "Adult"

#             gender_raw = row.get(f"{s}_Gender")
#             if pd.notna(gender_raw):
#                 gender = map_gender(gender_raw)
#             else:
#                 t_col = [c for c in df.columns if c.startswith(f"{s}_Name of your Quran")]
#                 teacher = row.get(t_col[0]) if t_col else "N/A"
#                 gender = infer_gender(teacher)
...
#             adult_sub = subset[subset["Entry Type"] == "Adult"].copy() if "Entry Type" in subset.columns else subset.iloc[0:0].copy()
#             child_sub = subset[subset["Entry Type"] != "Adult"].copy() if "Entry Type" in subset.columns else subset.copy()
...
#                 adult_sub = gen_sub[gen_sub["Entry Type"] == "Adult"].copy() if "Entry Type" in gen_sub.columns else gen_sub.iloc[0:0].copy()
#                 child_sub = gen_sub[gen_sub["Entry Type"] != "Adult"].copy() if "Entry Type" in gen_sub.columns else gen_sub.copy()

#                 if not child_sub.empty:
#                     child_sub.insert(0, "Count", range(1, len(child_sub) + 1))
#                     title = f"{grade} - {gender.upper()}"
#                     tables[title] = child_sub[["Count", "First Name", "Last Name", "Email"]]
#                 if not adult_sub.empty:
#                     adult_sub.insert(0, "Count", range(1, len(adult_sub) + 1))
#                     title = f"{grade} - {gender.upper()} - ADULT"
#                     tables[title] = adult_sub[["Count", "First Name", "Last Name", "Email"]]
...
#             {"bold": True, "font_size": 24, "font_color": "#1F4E78", "align": "left"}
#         )
#         used_sheet_names = set()

#         def safe_sheet_name(title: str) -> str:
#             base = re.sub(r"[:*?/\\\[\]]", "", title)[:31]
#             name = base
...
#             sheet_name = safe_sheet_name(sheet_title)
#             startrow = 3 if sheet_title != "Category_Counts" else 0
...
#     match_cols = [c for c in ["First Name", "Last Name"] if c in df.columns]
#     if not match_cols:
#         return df

#     def row_match(row):
#         haystack = " ".join(str(row[c]).lower() for c in match_cols)
...
# st.write("Upload the registration Excel file to generate all category tables.")

# uploaded = st.sidebar.file_uploader("Upload Excel/CSV file", type=["xlsx", "xls", "csv"])
...
#     c
#     for c in df.columns
#     if re.search(r"(location|city|center|centre|masjid|mosque|area)", str(c), re.I)
# ]
# results_df = extract_participants(df)
# if results_df.empty:
#     st.warning("No participant data found. Check your column headers.")
#     st.stop()

# tables = build_reports(results_df)
# all_categories = sorted(results_df["Grade"].unique(), key=get_sort_key)
...
# )
# apply_category = st.sidebar.button("Apply Category")
...
#     key="search_query",
# )
# apply_search = st.sidebar.button("Apply Search")

# if "applied_categories" not in st.session_state:
#     st.session_state.applied_categories = []
# if "applied_search" not in st.session_state:
#     st.session_state.applied_search = ""

# if apply_category:
#     st.session_state.applied_categories = selected_categories
# if apply_search:
#     st.session_state.applied_search = search_query

# tables_tab, summary_tab = st.tabs(["Preview Tables", "Summary Chart"])

# with summary_tab:
#     st.subheader("Summary Chart")
#     counts_df = tables["Category_Counts"].copy()
#     counts_df = counts_df[counts_df["Category"] != "Total"]
#     st.bar_chart(counts_df, x="Category", y="Count")

#     st.subheader("Participants by City")
...
#             index=0,
#         )
#         loc_series = df[selected_location_col].fillna("Unknown").astype(str).replace("", "Unknown")
...
#         st.dataframe(loc_counts, width="stretch")
#     else:
#         st.info("No city/location columns found in the uploaded file.")

# with tables_tab:
#     st.subheader("Preview Tables")
#     st.markdown("**Grouped View (All Participants)**")
#     combined = results_df.copy()
#     if st.session_state.applied_categories:
#         combined = combined[combined["Grade"].isin(st.session_state.applied_categories)]
#     combined_view = combined[["Grade", "Gender", "First Name", "Last Name", "Email"]]
#     combined_view = filter_matches(combined_view, st.session_state.applied_search)
#     st.caption(f"Matches: {len(combined_view)}")
#     combined_view = highlight_matches(combined_view, st.session_state.applied_search)
#     st.dataframe(combined_view, width="stretch")

#     for title, data in tables.items():
#         with st.expander(title, expanded=False):
#             view = filter_matches(data, st.session_state.applied_search)
#             view = highlight_matches(view, st.session_state.applied_search)
#             st.dataframe(view, width="stretch")

# excel_bytes = tables_to_excel_bytes(tables)
# download_name = f"Quran_Contest_Final_Report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
